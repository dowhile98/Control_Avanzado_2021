...
{
  {
    /* Output and update for function-call system: '<Root>/ISR lectura rads//s' */
    {
      ...
      char_T b_p[9];
      ControlMRAC_DW.rads = ControlMRAC_DW.pulses * 60.0 / 1.2 *
        6.2831853071795862 / 60.0;
      ControlMRAC_DW.pulses = 0.0 * ControlMRAC_DW.rads;
      for (i = 0; i < 15; i++) {
        buffer[i] = 0U;
      }

      ControlMRAC_B.Buff = getBuffPtr(buffer);
      for (i = 0; i < 9; i++) {
        b_p[i] = b[i];
      }

      ControlMRAC_B.NbChar = sprintf(buffer, b_p, ControlMRAC_DW.rads);

      {
        if (ControlMRAC_B.NbChar != 0) {
          /* Copy data to send. */
          TX_CopyData(&USART2_TxConf, (uint8_t*)ControlMRAC_B.Buff, (uint16_t)
                      ControlMRAC_B.NbChar);
        }

        if (USART2_TxConf.nb2Send) {
          ControlMRAC_B.USART_Send = 0;
          USART2_TX_SendData(&USART2_TxConf);
        }

        if (USART2_TxConf.txStatus != SERIAL_TX_ON) {
          ControlMRAC_B.USART_Send = USART2_TxConf.nbSent;
        }
      }

      ControlMRAC_DW.stateControl = 1.0;
    }
  }
}
